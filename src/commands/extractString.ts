...
import { extractHardStrings, generateKeyFromText, Config, CurrentFile, DetectionResult, Telemetry, TelemetryKey, Global } from '~/core'
...
function ExtractIngore(text: string, document?: TextDocument) {
  if (document) {
    const path = relative(Global.getRootpathOf(document.uri), document.uri.fsPath)
