...
export class Global {
  ...
  private static _currentProject: ResolvedMonorepoProjectConfig | undefined
  ...
  static getRootpathOf(resource?: TextDocument['uri']) {
    if (resource?.scheme === 'file') {
      const folder = workspace.getWorkspaceFolder(resource)
      if (folder)
        return this.resolveRootContext(folder, resource.fsPath).rootpath
    }

    return this._rootpath
  }

  private static isPathInside(filepath: string, rootpath: string) {
  ...
  private static resolveRootContext(folder: WorkspaceFolder, filepath?: string) {
    const projects = Config.getProjectsInScope(folder)
      ...
      .map(({ project, index }): ResolvedMonorepoProjectConfig => {
        ...
        const rootpath = resolve(folder.uri.fsPath, root)
        return {
          ...project,
  ...
  private static async updateRootPath(force = false) {
    const editor = window.activeTextEditor
    let rootpath = ''
    let currentProject: ResolvedMonorepoProjectConfig | undefined
    ...
    if (resource.scheme === 'file') {
      const folder = workspace.getWorkspaceFolder(resource)
      if (folder) {
        const context = this.resolveRootContext(folder, resource.fsPath)
    ...
    if (!rootpath && workspace.workspaceFolders?.[0]) {
      const context = this.resolveRootContext(workspace.workspaceFolders[0])
      this._currentWorkspaceFolder = context.workspaceFolder
      currentProject = context.project
      rootpath = context.rootpath
    }
    else if (!rootpath && workspace.rootPath) {
      rootpath = workspace.rootPath
    }

    if (rootpath && (force || rootpath !== this._rootpath || currentProject?.index !== this._currentProject?.index)) {
      this._rootpath = rootpath
      this._currentProject = currentProject
      Config.currentProject = currentProject
      Config.currentProjectScope = this._currentWorkspaceFolder

      Log.divider()
      const projectName = currentProject ? ` (${currentProject.name})` : ''
      Log.info(`💼 Workspace root changed to "${rootpath}"${projectName}`)
  ...
  static async update(e?: ConfigurationChangeEvent) {
    ...
    let reload = false
    if (e) {
      if (e.affectsConfiguration(`${EXT_NAMESPACE}.projects`)) {
        await this.updateRootPath(true)
        return
      }
