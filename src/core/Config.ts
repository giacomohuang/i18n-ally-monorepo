...
import { KeyStyle, DirStructureAuto, SortCompare, TargetPickingStrategy, ResolvedMonorepoProjectConfig, MonorepoProjectConfig } from '.'
...
export class Config {
  ...
  static _currentProject: ResolvedMonorepoProjectConfig | undefined
  static _currentProjectScope: ConfigurationScope | undefined

  static get root() {
    return this._currentProject?.rootpath || workspace.rootPath!
  }
  ...
  static set currentProject(project: ResolvedMonorepoProjectConfig | undefined) {
    this._currentProject = project
  }

  static get currentProjectScope() {
    return this._currentProjectScope
  }

  static set currentProjectScope(scope: ConfigurationScope | undefined) {
    this._currentProjectScope = scope
  }

  static get rawProjects(): MonorepoProjectConfig[] {
    return this.getProjectsInScope()
  }

  static getProjectsInScope(scope?: ConfigurationScope | undefined): MonorepoProjectConfig[] {
    const projects = this.getConfig<MonorepoProjectConfig[]>('projects', scope, false)
  ...
  static get projects(): MonorepoProjectConfig[] {
    const projects = this.rawProjects
  ...
  static get _localesPaths(): string[] | undefined {
    ...
    return this.normalizeLocalesPaths(paths)
  }
  ...
  // config
  private static getProjectConfig<T = any>(key: string): T | undefined {
    const value = this._currentProject?.[key]
    if (value !== undefined)
      return value as T
  }

  private static getConfig<T = any>(key: string, scope?: ConfigurationScope | undefined, projectOverride = true): T | undefined {
    if (projectOverride) {
      const projectValue = this.getProjectConfig<T>(key)
  ...
  private static normalizeLocalesPaths(paths: string | string[] | undefined): string[] | undefined {
  ...
  private static async setConfig(key: string, value: any, isGlobal = false) {
    if (this._currentProject && PROJECT_CONFIG_KEYS.has(key))
      return await this.setProjectConfig(key, value)
  ...
  private static async setProjectConfig(key: string, value: any) {
    const projects = this.getProjectsInScope(this._currentProjectScope).map(project => ({ ...project }))
    const project = this._currentProject
    if (!project || !projects[project.index])
      return

    if (value === undefined)
      delete projects[project.index][key]
    else
      projects[project.index][key] = value

    return await workspace
      .getConfiguration(EXT_NAMESPACE, this._currentProjectScope)
      .update('projects', projects, false)
  }
