...
export class Analyst {
  private static _cache: Record<string, KeyOccurrence[] | null> = {}
  ...
  private static get cacheKey() {
    return Global.rootpath
  }

  private static get cache() {
    return this._cache[this.cacheKey] || null
  }

  private static set cache(value: KeyOccurrence[] | null) {
    this._cache[this.cacheKey] = value
  }
  ...
  static invalidateCacheOf(filepath: string) {
    if (this.cache)
      this.cache = this.cache.filter(o => o.filepath !== filepath)
  }
  ...
  private static async updateCache(doc: TextDocument) {
    ...
    const rootpath = Global.getRootpathOf(doc.uri)
    if (rootpath !== Global.rootpath)
      return
  ...
  static async getAllOccurrences(targetKey?: string, useCache = true) {
    ...
    if (!this.cache) {
      ...
      this.cache = occurrences
    }

    if (targetKey)
      return this.cache!.filter(({ keypath }) => keypath === targetKey)
    return this.cache!
  }
