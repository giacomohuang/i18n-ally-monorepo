...
export class VueSfcLoader extends Loader {
  ...
  async load() {
    ...
    const meta = this._meta = squeeze(Global.getRootpathOf(this.uri), this.getSFCFileInfo(doc))
  ...
  async write(pendings: PendingWrite | PendingWrite[]) {
    ...
    const doc = await workspace.openTextDocument(this.uri)
    const [file] = infuse(Global.getRootpathOf(this.uri), this.getSFCFileInfo(doc), this._meta)
